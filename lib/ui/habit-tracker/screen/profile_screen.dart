...
class _ProfileScreenState extends ConsumerState<ProfileScreen> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => _loadBiometricEnabled());
  }
                      ...
                      children: [
                        Expanded(
                          child: Column(
                            ...
                            children: const [
                              Text('Enable Fingerprint Login', style: TextStyle(fontWeight: FontWeight.w700)),
                              SizedBox(height: 6),
                              Text('Use fingerprint to quickly unlock the app', style: TextStyle(color: Colors.grey, fontSize: 12)),
                            ],
                          ),
                        ),
                        Switch(
                          ...
                          onChanged: (val) async {
                            ...
                            if (val) {
                              ...
                              if (!available || types.isEmpty) {
                                if (!mounted) return;
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Biometric not available or not enrolled on this device')),
                                );
                                return;
                              }
                              ...
                              if (!authResult.success) {
                                if (!mounted) return;
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(content: Text(authResult.errorMessage ?? 'Authentication failed')),
                                );
                                return;
                              }
