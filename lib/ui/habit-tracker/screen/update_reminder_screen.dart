...
class UpdateReminderScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<UpdateReminderScreen> createState() =>
      _UpdateReminderScreenState();
}
...
class _UpdateReminderScreenState extends ConsumerState<UpdateReminderScreen> {
  ...
  void initState() {
    ...
    _isEnabled = widget.reminderSetting.isEnabled;
    _repeatDaily = widget.reminderSetting.repeatDaily;
    _soundEnabled = widget.reminderSetting.isSoundEnabled;
    _vibrationEnabled = widget.reminderSetting.isVibrationEnabled;
    _snoozeDuration = widget.reminderSetting.snoozeDuration;
    if (!_snoozeOptions.contains(_snoozeDuration)) {
      _snoozeDuration = 10;
    }
    _selectedTime = TimeOfDay.fromDateTime(widget.reminderSetting.time);
  }
  ...
  Future<void> _save() async {
    ...
    final reminderId = int.tryParse(widget.reminderSetting.id);
    if (reminderId == null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Reminder ID tidak valid')));
      return;
    }
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Reminder berhasil diupdate')),
      );
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Gagal update reminder: $e')));
    } finally {
